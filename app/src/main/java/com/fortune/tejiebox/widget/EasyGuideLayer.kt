...
typealias OnGuideShowListener = (Boolean) -> Unit
typealias OnGuideViewAttachedListener = (View, EasyGuideLayer.Controller) -> Unit
typealias OnDrawHighLightCallback = (canvas: Canvas, rect: RectF, paint: Paint) -> Unit
typealias OnHighLightClickListener = (EasyGuideLayer.Controller) -> Unit
typealias OnGuideViewOffsetProvider = (Point, RectF, View) -> Unit
...
class EasyGuideLayer private constructor(private val anchor: View) {
    ...
    companion object {
        ...
        fun with(activity: Activity) = with(activity.findViewById<View>(R.id.content))
    }

    class GuideLayout(context: Context?) : FrameLayout(context!!), Controller,
        ...
        private fun resetChildLayoutParams(item: GuideItem, rect: RectF, child: View) {
            ...
            val point: Point = when (item.getGravity()) {
                Gravity.LEFT or Gravity.TOP -> {
                    Point(rect.left.toInt() - child.width, rect.top.toInt() - child.height)
                }

                Gravity.RIGHT or Gravity.TOP -> {
                    Point(rect.right.toInt(), rect.top.toInt() - child.height)
                }

                Gravity.LEFT or Gravity.BOTTOM -> {
                    Point(rect.left.toInt() - child.width, rect.bottom.toInt())
                }
                ...
                Gravity.TOP -> {
                    Point(rect.left.toInt(), (rect.top - child.height).toInt())
                }

                Gravity.BOTTOM -> {
                    Point(rect.left.toInt(), rect.bottom.toInt())
                }

                Gravity.LEFT -> {
                    Point((rect.left - child.width).toInt(), rect.top.toInt())
                }

                Gravity.RIGHT -> {
                    Point(rect.right.toInt(), rect.top.toInt())
                }
        ...
        private fun getRectFromItem(item: GuideItem): RectF {
            ...
            return RectF(
                (viewRect.left - guideRect.left - item.padding).toFloat(),
        ...
        override fun onTouchEvent(event: MotionEvent): Boolean {
            ...
            when (event.action) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    if (event.x - down.x > touchSlop || event.y - down.y > touchSlop) downPoint =
                        null
                }
 ...
 * */
class GuideItem private constructor(
    val rect: RectF? = null,
    val view: View? = null,
    val padding: Int = 0
) {
    ...
    fun setDrawable(@DrawableRes drawableRes: Int): GuideItem {
        ...
        this.drawable = MyApp.getInstance().resources.getDrawable(drawableRes)
        this.layout = 0
        return this
    }
